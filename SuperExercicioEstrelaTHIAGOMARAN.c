...
int main(){
    ...
    while(i>=0 && i<qtd_estrelas){  //enquanto i for maior que 0 (em algum momento, ele vai zerar), i for menor que qtd_estrelas     
        if(estrela[i]>0){           //se for maior que 0
            if(atacadas[i]==0){     //se atacadas[i] for 0 (não foi atacado ainda) - todos os [i] são 0. então viram 1 para definir "atacado" | é basicamente boolean
                atacadas[i]=1;      //define atacadas[i] como 1 (atacado) | define o bool para true
